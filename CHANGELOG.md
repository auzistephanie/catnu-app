...
- 2026-09-03：**持續使用體驗 sprint：每週故事＋互動歷史＋自訂每日目標＋備份安全感** ——
  - 分析頁加入 deterministic「今週關係故事」：按近 7 日記錄整理互動數、正面率變化、今週一刻同下一個溫柔建議；資料不足時顯示誠實暖場，並標明唔係醫療或行為診斷，冇用 AI 分析。
  - 記錄頁加入最近互動同完整 timeline，可按貓篩選、展開詳情、刪除後 10 秒內復原；修正由 history 撳頂部 tab 後畫面仍停留子頁嘅 navigation bug。
  - 每日 3 個任務維持不變，但用戶可揀 1／2／3 個做達標線，預設 2；達標只加一次 streak，其餘任務當 bonus。新增 controls 補齊 `aria-pressed` 狀態。
  - 檔案頁加入 30 日備份狀態、下載後即時更新、還原前日期／貓數／互動數／里程碑預覽，同公開 beta local-only 說明；landing 同步講清楚互動歷史、每週故事、預設兩個任務及 JSON 備份／還原。
  - 範圍維持 local-only，冇加入 login、雲端同步、AI、相簿或健康記錄。
  - 驗證：`node --test tests/*.test.mjs` **67/67 全綠**；390px 實際完成快速記錄 → timeline／詳情 → 分析故事 → 2 個任務達標 → 備份下載／狀態更新，另以 320px 驗 app、390px／320px 驗 landing，全部無橫向 scroll、零 console warning/error。內置瀏覽器未能打開隱藏 file chooser，還原預覽以 pure-logic 自動測試及 UI 靜態檢查覆核，未做瀏覽器實際選檔。
