...
- **冇背景 auto-push daemon**（2026-07-16 起，見 stephanie-personal/CLAUDE.md「Git Push」段正本）：Cowork 雲端改完 → `device_commit_files` 寫返 Mac 之後，唔會自動 push，要手動雙擊 `stephanie-personal/scripts/push-now.command`（跑晒 autopush-registry 入面全部 repo，包括呢個），或者喺呢個 repo 入面直接跑上面嗰句 `github_push.py`。
